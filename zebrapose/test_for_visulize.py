...
def main(configs):
    ...
    for batch_idx, (data, entire_masks, masks, Rs, ts, Bboxes, class_code_images, cam_Ks) in enumerate(tqdm(test_loader)):
        ...
        class_code_images = class_code_images.detach().cpu().numpy().transpose(0, 2, 3, 1).squeeze(axis=0)
        ...
        for counter, (r_GT, t_GT, Bbox, cam_K) in enumerate(zip(Rs, ts, Bboxes, cam_Ks)):
            ...
            if success:     
                if True:
                    ...
                    show_ims = [x,]
                    show_titles = ["image",]
                    grid_show(show_ims, show_titles, row=1, col=1, save_path=os.path.join(debug_image_dir, "img.jpg"))
                    ...
                    pred_code_images[np.where(pred_masks==0)] = 0.8

                    show_ims = []
                    show_titles = []
                    for i in range(16):
                        show_ims.append((pred_code_images[counter][:, :, [i]]).repeat(3,2)*255.)
                    ...
                    class_code_images[np.where(gt_mask==0)]=0.8
                    show_ims = []
                    show_titles = []
                    for i in range(16):
                        show_ims.append(class_code_images[:, :, [i]].repeat(3,2) * 255)
                        show_titles.append("bit " + str(i))
                    ...
                    pred_code_prob[np.where(pred_masks==0)] = 0.8
                    pred_code_prob = pred_code_prob[counter]
                    show_ims = []
                    show_titles = []
                    for i in range(16):
                        show_ims.append(pred_code_prob[:, :, [i]].repeat(3,2) * 255)
                        show_titles.append("bit " + str(i))
                    grid_show(show_ims, show_titles, row=4, col=4,
